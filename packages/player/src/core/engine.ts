...
import { findFirstIndexAtOrAfter, floatToInt16, throwIfAborted } from '@be-music/utils/core';
...
export async function manualPlay(json: BeMusicJson, options: PlayerOptions = {}): Promise<PlayerSummary> {
  ...
  const autoScratchNotes = autoScratchEnabled
    ...
    : [];
  let autoScratchCursor = findFirstIndexAtOrAfterSeconds(autoScratchNotes, playbackStartSeconds);
  let scorableMissCursor = findFirstIndexAtOrAfterSeconds(scorableNotes, playbackStartSeconds);
  let landmineExpireCursor = findFirstIndexAtOrAfterSeconds(landmineNotes, playbackStartSeconds);
  let invisibleExpireCursor = findFirstIndexAtOrAfterSeconds(invisibleNotes, playbackStartSeconds);
  let realtimeAudioVolumeEventIndex = findFirstIndexAtOrAfterSeconds(realtimeAudioVolumeEvents, playbackStartSeconds);
  let remainingScorableNotes = remainingScorableCount;
  let remainingLandmineNotes = remainingLandmineCount;
  let remainingInvisibleNotes = remainingInvisibleCount;
  let nonPlayableRealtimeAudioTriggerIndex = findFirstIndexAtOrAfterSeconds(
    nonPlayableRealtimeAudioTriggers,
    playbackStartSeconds,
  );
...
async function createAudioSessionIfEnabled(
  ...
  return {
    ...
    triggerEvent: (event: BeMusicEvent) => triggerAudioEvent(event),
    stopChannel:
      mode === 'manual'
        ? (channel: string) => {
            if (paused) {
