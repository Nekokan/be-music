...
/**
 * Composition mode for a BGA frame.
 *
 * - `'base'` — bottommost track. Every pixel renders, including pure black, since there's nothing visible behind it.
 * - `'layer'` — BMS-style overlay (`#xxx07`). Pure-black BMP / JPEG / video pixels render transparent so the foreground
 ...
 *   transparent." Honor the PNG alpha channel but render BMP / JPEG / video black pixels as black, since bmson layer artwork
...
let convertImageToSpecFrameWorker: WorkerizedSpecFrameConverter | undefined;
...
export class BgaAnsiRenderer {
  ...
  private resolveActiveState(currentSeconds: number): {
    ...
    const baseCue = pickActiveBgaCue(this.baseTimeline, currentSeconds);
    const layerCue = pickActiveBgaCue(this.layerTimeline, currentSeconds);
    const layer2Cue = pickActiveBgaCue(this.layer2Timeline, currentSeconds);
    const poorCue = pickActiveBgaCue(this.poorTimeline, currentSeconds);
...
export async function createBgaAnsiRenderer(
  ...
  const {
    ...
    poor: poorTimeline,
    layer: layerTimeline,
    layer2: layer2Timeline,
  } = buildBgaTimelines(json, resolver);
...
async function convertImageToSpecFrameOffThread(
  ...
  const activeWorker = resolveConvertImageToSpecFrameWorker();
  try {
    const frame = await invokeWorkerizedFunction(activeWorker, [image, mode, resizeAlgorithm], {
      ...
      onAbort: () => {
        if (convertImageToSpecFrameWorker === activeWorker) {
          ...
          convertImageToSpecFrameWorker = undefined;
        }
      },
    });
...
function resolveConvertImageToSpecFrameWorker(): WorkerizedSpecFrameConverter {
  convertImageToSpecFrameWorker ??= createConvertImageToSpecFrameWorker();
  return convertImageToSpecFrameWorker;
}
...
function isOpaquePixel(r: number, g: number, b: number, a: number, format: ImageFormat, mode: FrameMode): boolean {
  ...
  if (format === 'bmp' || format === 'jpeg' || format === 'video') {
    return !(r === 0 && g === 0 && b === 0);
  }
  if (format === 'png') {
    return a > TRANSPARENT_ALPHA_THRESHOLD;
